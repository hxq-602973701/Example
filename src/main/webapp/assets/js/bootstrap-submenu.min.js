...
!function (a) {
    ...
    return b.prototype = {
        init: function () {
            this.$element.on({
                "click.bs.dropdown": a.proxy(this.click, this),
                keydown: a.proxy(this.keydown, this)
            }), this.$menu.on("hide.bs.submenu", a.proxy(this.hide, this)), this.$items.on("keydown", a.proxy(this.item_keydown, this)), this.$menu.nextAll(c + ":first:not(.dropdown-submenu)").children("a").on("keydown", a.proxy(this.next_keydown, this))
        }, click: function (a) {
            ...
            if (-1 != a.inArray(b.keyCode, [32, 38, 40]) && b.preventDefault(), -1 != a.inArray(b.keyCode, [13, 32]))this.toggle(); else if (-1 != a.inArray(b.keyCode, [27, 38, 40]))if (b.stopPropagation(), 27 == b.keyCode)this.$menu.hasClass("open") ? this.close() : (this.$menus.trigger("hide.bs.submenu"), this.$drop.removeClass("open").children("a").trigger("focus")); else {
                var c = this.$main.find("li:not(.disabled):visible > a"), d = c.index(b.target);
